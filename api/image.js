const chromium = require('chrome-aws-lambda'),
  ...
  fs = require('fs'),
  production = process.env.NODE_ENV === 'productionx',
  ...
  response.setHeader('Access-Control-Allow-Origin', '*')
    ...
    console.info('🛠 ', `Hostname: ${hostname}`);
      ...
      const message = !settings.language ? 'Language not specified' : `Unknown language '${settings.language}'`;
      ...
      const message = `Unknown theme '${settings.theme}'`;
			console.info('❌ ', message);
			return sendErrorResponse(response, {
				message,
				themes,
    ...
    !settings.theme && (settings.theme = themes[0]);
    !settings.lineNumbers && (settings.lineNumbers = false);
    ...
    !settings.showBackground && (settings.showBackground = true);
    ...
    settings.trimmedCodeSnippet = settings.language === 'javascript' ? 
      beautify(settings.code, { indent_size: 2, space_in_empty_paren: true }) : trimLineEndings(settings.code);
    ...
    if (settings.backgroundImage )
      queryParams.set('backgroundImage', settings.backgroundImage);
    if (settings.backgroundColor )
      queryParams.set('backgroundColor', settings.backgroundColor);
    if (settings.padding )
		  queryParams.set('padding', settings.padding);
